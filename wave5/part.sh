...
#        echo "  type: object" >>$output
    ...
    echo "$space    description: |"|tee -a $inputModelFile $outputModelFile>>/dev/null
        ...
        then
            ...
            paste -d"|" bqel rest>q
            #echo "crbq|element: $crbq|$element"
...
sdname=$1
serviceDomain="$(tr '[:upper:]' '[:lower:]' <<< ${sdname:0:1})${sdname:1}"
sed -e "s/ServiceDomain/SD$1/g" -e "s/serviceDomain/$serviceDomain/g" SDGenericModel.yaml>>$output
cat $catFile >>$output
